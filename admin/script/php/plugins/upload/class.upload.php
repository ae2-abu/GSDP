 ...
 * to its `".$tt."` location (alternatively, you can instanciate
 ...
 *   - fixed a bug when processing more than once the same uploaded file. If there is an open_basedir restriction, the class now creates a `".$tt."` file for the first call to process(). This file will be used for subsequent processes, and will be deleted upon calling clean()</li>
 *  <li><b>v 0.22</b> 16/12/2006<br>
 *   - added automatic creation of a `".$tt."` file if the upload directory is not within open_basedir<br>
...
class upload {
    ...
    function init() {
        ...
        $this->file_new_name_body       = $_POST['question_id'];     // replace the name body
     ...
     *
     * The constructor will check if the file has been uploaded in its `".$tt."` location, and
    ...
    function upload($file, $lang = 'en_GB') {
       ...
       $this->file_src_name      = '';
        ...
        $this->translation['uploaded_no_tmp_dir']         = 'File upload error (missing a `".$tt."` folder).';
        ...
        $this->translation['temp_file']                   = 'Can\'t create the `".$tt."` file. Can\'t carry on a process.';
     ...
     * The original uploaded file remains intact in its `".$tt."` location, so you can use {@link process} several times.
    ...
    function process($server_path = null) {
        ...
        if ($this->processed) {
            // if we have already moved the uploaded file, we use the `".$tt."` copy as source file, and check if it exists
    ...
    /**
     * Deletes the uploaded file from its `".$tt."` location
     *
     * When PHP uploads a file, it stores it in a `".$tt."` location.
     ...
     * If there is open_basedir restrictions, the uploaded file is in fact a `".$tt."` file
     *
