...
include('class.upload.php');
        ...
        $bannerTitle = mysql_prep($_POST['title']);
        $bannerDesc = mysql_prep($_POST['desc']);
     ...
     ////////////////////////////////////////////////////////////////////////////////////////////////
                    ...
                    // set variables
        $dir_dest = (isset($_GET['dir']) ? $_GET['dir'] : BANNER_DIR_FULL);
        $dir_pics = (isset($_GET['pics']) ? $_GET['pics'] : $dir_dest);
            ...
            if ($handle->uploaded) {
                ...
                $handle->file_new_name_body = "BAN".uniqid()."U".$_SESSION['user_id']."T".time(); 
                ...
                if ($handle->processed) {
                 ...
                 $stmt=$con->prepare("INSERT into banner(title,b_desc,file_name,biz_id) values(?,?,?,?)") or die($con->error);
                 $stmt->bind_param("ssss",$bannerTitle,$bannerDesc,$handle->file_dst_name,$_SESSION['company_id']) or die($con->error); 
                 $stmt->execute() or die($con->error);
                 $last_insert=$stmt->insert_id;
                 $time=time();
                     if( $stmt->affected_rows>0){ 
                             $arrRes = array('success' =>1 ,'banner_id'=>$last_insert,'post_time'=>$time );
